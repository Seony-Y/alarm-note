...
달력에서 날짜를 선택하면 앱을 열지 않고 해당 날짜의 알람을 최대 3개까지 확인할 수 있습니다. 알람 상태도 위젯에서 바로 켜고 끌 수 있으며 월 이동과 일정 추가를 지원합니다.

![Android 홈 화면 위젯 미리보기](docs/preview-06.svg)
...
앱을 한 번 실행해 일정을 불러온 다음 홈 화면을 길게 눌러 **위젯 > 하루알람**을 선택합니다. 로그인하지 않은 상태에서는 로그인 안내를 표시하며, 누르면 앱의 간편 로그인 화면이 열립니다. 로그인 후에는 왼쪽 월간 달력에서 날짜와 월을 이동하면 오른쪽에 해당 날짜의 알람이 최대 3개 표시됩니다. 상태 버튼은 앱을 열지 않고 알람을 바로 켜거나 끄며, 알람 행은 해당 월의 알람 관리 화면을 엽니다. 추가 버튼은 선택한 날짜의 새 일정 화면을 엽니다. 일정 추가, 수정, 완료 또는 알람 상태 변경 시 내용이 갱신됩니다.
